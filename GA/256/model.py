...
def get_extension(verbose=False):
    ...
    _EXT = load(
        name=f"brnn_cuda_ext_v3_sm{arch}",
        sources=[src],
        extra_cuda_cflags=[
            "-O3",
            "--use_fast_math",
...
def pick_mutation(num_ff, target=None, index=None):
    ...
    if target is not None:
        ...
        raise ValueError(
            "target must be one of ['initial', 'embed', 'ff', 'head'] "
...
def pack_initial(x):
    require_shape(x, (256,), "initial")
    return pack_pm1_rows(x.view(1, 256)).view(8)


def unpack_initial(x):
    require_shape(x, (8,), "initial_p")
    return unpack_pm1_rows(x.view(1, 8), 256).view(256)


def pack_embed(x):
    require_shape(x, (128, 128), "embed")
    return pack_pm1_rows(x)


def unpack_embed(x):
    require_shape(x, (128, 4), "embed_p")
    return unpack_pm1_rows(x, 128)
...
def pack_ff(x):
    if x.ndim != 3:
        raise ValueError("ff must have shape [num_ff, 256, 256]")

    if x.shape[1:] != (256, 256):
        raise ValueError("ff must have shape [num_ff, 256, 256]")

    if x.shape[0] == 0:
        return torch.empty((0, 8, 256), device=x.device, dtype=torch.int32)
...
def unpack_ff(x):
    if x.ndim != 3:
        raise ValueError("ff_p must have shape [num_ff, 8, 256]")

    if x.shape[1:] != (8, 256):
        raise ValueError("ff_p must have shape [num_ff, 8, 256]")

    if x.shape[0] == 0:
        return torch.empty((0, 256, 256), device=x.device, dtype=torch.int8)
...
class ReferenceBRNN(nn.Module):
    ...
    def __init__(self, source=None, num_ff=None):
        ...
        self.register_buffer("initial", torch.empty((256,), device=device, dtype=torch.int8))
        self.register_buffer("embed", torch.empty((128, 128), device=device, dtype=torch.int8))
        self.register_buffer("ff", torch.empty((self.num_ff, 256, 256), device=device, dtype=torch.int8))
        self.register_buffer("head", torch.empty((128, 128), device=device, dtype=torch.int8))

        if source is None:
            self.initial.copy_(random_pm1((256,), device))
            self.embed.copy_(random_pm1((128, 128), device))
            self.ff.copy_(random_pm1((self.num_ff, 256, 256), device))
            self.head.copy_(random_pm1((128, 128), device))
        else:
    ...
    def _copy_from(self, source):
        if has_attrs(source, ["initial", "embed", "ff", "head"]):
            require_shape(source.initial, (256,), "source.initial")
            require_shape(source.embed, (128, 128), "source.embed")
            require_shape(source.ff, (self.num_ff, 256, 256), "source.ff")
            require_shape(source.head, (128, 128), "source.head")
        ...
        if has_attrs(source, ["initial_p", "embed_p", "ff_p", "head_p"]):
            require_shape(source.initial_p, (8,), "source.initial_p")
            require_shape(source.embed_p, (128, 4), "source.embed_p")
            require_shape(source.ff_p, (self.num_ff, 8, 256), "source.ff_p")
            require_shape(source.head_p, (4, 128), "source.head_p")
        ...
        raise TypeError(
            ...
            "`initial`, `embed`, `ff`, `head`, "
            "or KernelBRNN-like with buffers "
            "`initial_p`, `embed_p`, `ff_p`, `head_p`"
        )
    ...
    def logits_and_carry(self, x):
        ...
        if x.ndim == 1:
            require_shape(x, (256,), "x")
            x = x.view(1, 256)
            single = True

        if x.ndim != 2 or x.shape[1] != 256:
            raise ValueError("x must have shape [256] or [B, 256]")

        x = x.to(device=self.initial.device, dtype=torch.int8).contiguous()

        for layer in range(self.num_ff):
            x = self._sign(self._int_mm(x, self.ff[layer]))

        carry = x[:, :128].contiguous()
        read = x[:, 128:].contiguous()
    ...
    def advance(self, x, token):
        ...
        if x.ndim == 1:
            ...
            if token < 0 or token >= 128:
                raise ValueError("token must be in [0, 127]")
    ...
    def loss(self, tokens):
        ...
        x = self.initial.unsqueeze(0).expand(B, 256).clone().contiguous()
    ...
    def generate(self, prompt=None, activation=None, temperature=1.0):
        ...
        if activation is None:
            ...
            require_shape(activation, (256,), "activation")
...
class KernelBRNN(nn.Module):
    ...
    def __init__(self, source=None, num_ff=None):
        ...
        self.register_buffer("initial_p", torch.empty((8,), device=device, dtype=torch.int32))
        self.register_buffer("embed_p", torch.empty((128, 4), device=device, dtype=torch.int32))
        self.register_buffer("ff_p", torch.empty((self.num_ff, 8, 256), device=device, dtype=torch.int32))
        self.register_buffer("head_p", torch.empty((4, 128), device=device, dtype=torch.int32))

        if source is None:
            initial = random_pm1((256,), device)
            embed = random_pm1((128, 128), device)
            ff = random_pm1((self.num_ff, 256, 256), device)
            head = random_pm1((128, 128), device)
