...
# ============================================================
# Stuck-neuron detection and unsticking (FF threshold repair)
# ============================================================
#
# An FF unit is "stuck" on a batch when its pre-threshold dot product
# never crosses its threshold, so the +1/-1 output is constant for the
# entire batch. With the convention `output = +1 iff dot > thresh`:
#
#     stuck always +1   <=>   thresh < min(dot)
#     stuck always -1   <=>   thresh >= max(dot)
#
# We "unstick" by moving the threshold the minimum amount needed to put
# at least one example on each side of the comparison:
#
#     stuck always +1   ->   thresh := min(dot)
#                            (now the example with dot == min gives -1)
#
#     stuck always -1   ->   thresh := max(dot) - 1
#                            (now the example with dot == max gives +1)
#
# A unit whose dot product is *constant* on the batch (min == max) is
# fundamentally unobservable on this batch; we leave it alone.

from model import D_MODEL, CARRY_DIM  # already importing ReferenceBRNN, KernelBRNN, etc.


@torch.no_grad()
def compute_ff_dot_ranges(ref_model: ReferenceBRNN, batch: torch.Tensor):
    """
    Run the unpacked reference model over the batch and record, for every
    FF layer and every output neuron, the minimum and maximum integer
    pre-activation (the dot product before thresholding) seen over all
    batch elements and all timesteps.

    Returns:
        mins: int32 tensor [num_ff, D_MODEL]
        maxs: int32 tensor [num_ff, D_MODEL]
    """
    device = ref_model.initial.device
    B, T = batch.shape
    num_ff = ref_model.num_ff

    INF = 10**9
    mins = torch.full((num_ff, D_MODEL), INF, dtype=torch.int32, device=device)
    maxs = torch.full((num_ff, D_MODEL), -INF, dtype=torch.int32, device=device)

    x = ref_model.initial.unsqueeze(0).expand(B, D_MODEL).clone().contiguous()

    for t in range(T):
        cur = x
        for layer in range(num_ff):
            preact = ReferenceBRNN._int_mm(cur, ref_model.ff[layer])  # [B, D_MODEL] int32
            mins[layer] = torch.minimum(mins[layer], preact.amin(dim=0).to(torch.int32))
            maxs[layer] = torch.maximum(maxs[layer], preact.amax(dim=0).to(torch.int32))
            cur = ReferenceBRNN._sign_with_thresh(preact, ref_model.ff_thresh[layer])

        carry = cur[:, :CARRY_DIM]
        x = torch.cat([carry, ref_model.embed[batch[:, t]]], dim=1).contiguous()

    return mins, maxs


@torch.no_grad()
def unstick_ff_neurons(ref_model: ReferenceBRNN, batch: torch.Tensor) -> int:
    """
    Find every FF neuron that is stuck on `batch` and adjust its threshold
    in-place to put it back on the boundary of the observed dot range.

    Returns the number of neurons whose threshold was changed.
    """
    mins, maxs = compute_ff_dot_ranges(ref_model, batch)
    thresh = ref_model.ff_thresh

    stuck_high = thresh < mins      # output always +1
    stuck_low  = thresh >= maxs     # output always -1
    can_unstick = mins < maxs       # any variation at all on this batch

    new_thresh = thresh.clone()
    new_thresh = torch.where(stuck_high & can_unstick, mins, new_thresh)
    new_thresh = torch.where(stuck_low  & can_unstick, maxs - 1, new_thresh)

    change_mask = (stuck_high | stuck_low) & can_unstick
    ref_model.ff_thresh.copy_(new_thresh)

    return int(change_mask.sum().item())
...
def save_checkpoint(
    ...
    # Convert to a (CPU/GPU) ReferenceBRNN so we can run the unstuck-detection
    # forward at full int32 precision and edit ff_thresh directly.
    ref_model = ReferenceBRNN(model)
    n_unstuck = unstick_ff_neurons(ref_model, batch)

    # Save the (now-repaired) reference state_dict on CPU for portability.
    state_dict_cpu = {
        key: value.detach().cpu()
        for key, value in ref_model.state_dict().items()
    }
    ...
    # Propagate the repaired thresholds back into the live KernelBRNN, so
    # iteration resumes from the unstuck state.
    model._copy_from(ref_model)

    now = datetime.now()
    print(f"\n>>> Checkpoint saved to {path} at step {step}")
    print(f">>> Unstuck {n_unstuck} FF neurons during this checkpoint")
    print(f">>> Time: {now.strftime('%Y-%m-%d %H:%M:%S')} ({int(now.timestamp())})")
...
def load_checkpoint(
    ...
    # strict=False so that older checkpoints (without ff_thresh) still load;
    # any missing buffers retain their constructor defaults (zeros for ff_thresh).
...
def genetic_alg_step(model: KernelBRNN, batch: torch.Tensor) -> Tuple[float, int]:
    ...
    while True:
        ...
        mutation = model.mutate()  # apply random mutation; returns descriptor
    ...
    while True:
        ...
        if args.checkpoint_every > 0 and (
            ...
            save_checkpoint(
                path=args.checkpoint_path,
                model=model,
                batch=batch, # to unstick neurons
                byte_to_id=byte_to_id,
                id_to_byte=id_to_byte,
                step=step,
                generation=generation,
