...
__global__ void brnn_forward_v4_kernel(
    ...
    const int32_t*  __restrict__ ff_thresh,      // [num_ff, 512]
    ...
    for (int t = 0; t < T; ++t) {
        // ------------------------------------------------------------
        // x = sign(x @ ff[layer] - thresh[layer]) for each layer
        ...
        for (int layer = 0; layer < num_ff; ++layer) {
            ...
            const int32_t* layer_thresh =
                ff_thresh + layer * D_MODEL;
            ...
            const int thresh = __ldg(layer_thresh + tid);
            const bool positive = dot > thresh;
...
torch::Tensor forward_cuda_v4(
    ...
    ff_thresh = ff_thresh.contiguous();
    ...
    TORCH_CHECK(ff_thresh.dim() == 2, "ff_thresh must be rank 2");
    TORCH_CHECK(ff_thresh.size(0) == num_ff,
                "ff_thresh shape must be [num_ff, 512]");
    TORCH_CHECK(ff_thresh.size(1) == D_MODEL,
                "ff_thresh shape must be [num_ff, 512]");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        ...
        "(D_MODEL=512, CARRY=384, READ=128, with FF thresholds)"
    );
}
